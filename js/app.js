...
(function () {
  ...
  /* ---------- 테마 ---------- */
  const THEMES = [
    ['classic', '클래식'], ['gummy', '구미베어'], ['nouveau', '아르누보'],
    ['midnight', '미드나잇'], ['goldleaf', '골드리프'], ['pastel', '파스텔'],
    ['neon', '네온'], ['sepia', '세피아'], ['woodcut', '우드컷'],
    ['sakura', '사쿠라'], ['ocean', '오션'], ['mono', '모노크롬']
  ];
  function initThemes() {
    const wrap = $('#themePicker');
    const saved = localStorage.getItem('tarot.theme') || 'classic';
  ...
  function setTheme(id, save = true) {
    ...
    $$('.theme-chip').forEach((b) => b.setAttribute('aria-pressed', String(b._id === id)));
  ...
  function fallbackHTML(card) {
    return '<div class="fallback"><div class="fb-mark">✦</div>' +
      '<div class="fb-name">' + card.name + '</div>' +
      '<div class="fb-ko">' + card.ko + '</div></div>';
  }
  ...
  function layoutFlow() {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      let t, op, z;
      if (off === 0) {
        t = 'translate3d(0,0,60px) rotateY(0deg) scale(1.06)';
        op = 1; z = 200;
        el.classList.add('is-center');
      } else {
        const x = sign * (150 + (abs - 1) * 54);
        const tz = -130 - (abs - 1) * 46;
        const ry = sign * -58;
        const sc = Math.max(0.8, 1 - abs * 0.05);
        t = `translate3d(${x}px,0,${tz}px) rotateY(${ry}deg) scale(${sc})`;
        op = abs <= 6 ? Math.max(0, 1 - abs * 0.16) : 0;
        z = 200 - abs;
      ...
      el.style.opacity = op;
      el.style.zIndex = z;
      el.style.pointerEvents = abs <= 6 ? 'auto' : 'none';
    }
    const c = CARDS[center];
    $('#flowMeta').innerHTML = '<strong>' + c.name + '</strong> · ' + c.ko +
      ' &nbsp;<span style="opacity:.7">(' + (center + 1) + '/' + CARDS.length + ')</span>';
  ...
  function initFlowControls() {
    ...
    stage.addEventListener('wheel', (e) => {
      ...
      if (Math.abs(acc) > 60) { gotoFlow(center + (acc > 0 ? 1 : -1)); acc = 0; e.preventDefault(); }
    }, { passive: false });
    let down = false, startX = 0, startC = 0;
    const press = (x) => { down = true; startX = x; startC = center; };
    const move = (x) => { if (down) { const d = Math.round((startX - x) / 60); gotoFlow(startC + d); } };
    const up = () => { down = false; };
    stage.addEventListener('mousedown', (e) => press(e.clientX));
    window.addEventListener('mousemove', (e) => move(e.clientX));
    window.addEventListener('mouseup', up);
    stage.addEventListener('touchstart', (e) => press(e.touches[0].clientX), { passive: true });
  ...
  const SPREADS = [
    ...
    { n: 3, label: '3장', sub: '과거·현재·미래' },
    { n: 5, label: '5장', sub: '십자' },
    { n: 7, label: '7장', sub: '말굽' },
    { n: 10, label: '10장', sub: '켈틱 크로스' }
  ];
  let drawCount = 3;
  function initCounts() {
  ...
  function render(result, spreadCount, { scroll = true } = {}) {
    ...
    result.picks.forEach((pick, idx) => {
      ...
      sum.innerHTML =
        ...
        (reading.authored ? ' · 손글' : '') + '</span></button>';
      ...
      sum.querySelector('.read-toggle').addEventListener('click', (e) => {
        ...
        e.currentTarget.firstChild.textContent = body.hidden ? '전체 해석 읽기' : '접기';
      });
  ...
  function doDraw() {
    ...
    const rec = window.TarotArchive.add({
      seed: result.seed, drawnAt: result.drawnAt,
      count: drawCount, picks: result.picks
    });
    refreshArchive();
    return rec;
  }
  ...
  function refreshArchive() {
    ...
    items.forEach((rec) => {
      ...
      el.querySelector('.a-del').addEventListener('click', () => {
        window.TarotArchive.remove(rec.id); refreshArchive();
      });
      list.appendChild(el);
    });
  }
  ...
  /* ---------- 부팅 ---------- */
  function init() {
    // 키워드 문자열을 카드에 미리 부착(렌더 편의).
    CARDS.forEach((c) => {
      c.keywordsUp = (c.keywords && c.keywords.up ? c.keywords.up : c.light).join(', ');
      c.keywordsRev = (c.keywords && c.keywords.rev ? c.keywords.rev : c.shadow).join(', ');
    });
    initThemes();
    buildFlow();
    initFlowControls();
